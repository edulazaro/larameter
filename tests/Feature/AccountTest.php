...
class AccountTest extends TestCase
{
    ...
    public function test_an_account_appears_on_first_sight_with_the_default_plan(): void
    {
        ...
        $this->assertSame(50, $org->usage()->remaining(), 'the tightest window');
    }
    ...
    public function test_the_tightest_window_is_the_one_that_binds(): void
    {
        ...
        $org->usage()->charge('thing', credits: 40);

        // Session had 50 and now has 10. The week had 200 and now has 160.
        $this->assertSame(10, $org->usage()->headroom());
    }
    ...
    public function test_the_balance_is_the_headroom_plus_what_was_bought(): void
    {
        ...
        $this->assertSame(550, $org->usage()->remaining());

        $org->usage()->charge('thing', credits: 30);

        $this->assertSame(520, $org->usage()->remaining());
    }
    ...
    public function test_the_plan_allowance_is_spent_before_anything_purchased(): void
    {
        ...
        $record = $org->usage()->charge('thing', credits: 50);

        $this->assertSame(50, $record->fresh()->credits_from_plan);
        $this->assertSame(0, $record->fresh()->credits_from_purchased);
        $this->assertSame(500, $org->usage()->account()->purchased_credits, 'what they paid for is untouched');
    }
    ...
    public function test_purchased_credits_are_not_counted_against_the_windows(): void
    {
        ...
        $org->usage()->charge('burn the session', credits: 50);
        $org->usage()->charge('carry on paying', credits: 300);
    ...
    public function test_credits_with_no_plan_at_all_is_a_valid_account(): void
    {
        ...
        $org->usage()->deposit(200);

        $this->assertSame(200, $org->usage()->remaining());
    ...
    public function test_an_overdraft_clamps_at_zero_and_stays_visible_in_the_split(): void
    {
        ...
        $record = $org->usage()->charge('thing', credits: 150)->fresh();

        $this->assertSame(0, $org->usage()->remaining());
        $this->assertSame(0, $org->usage()->account()->purchased_credits);
    ...
    public function test_upgrading_raises_the_ceiling_without_restarting_the_window(): void
    {
        ...
        $this->assertSame(1_200, $org->usage()->headroom());
    }
    ...
    public function test_a_usage_row_written_by_anything_at_all_moves_the_balance(): void
    {
        ...
        $this->assertSame(30, $org->usage()->remaining());
    }
    ...
    public function test_a_deposit_written_by_anything_at_all_moves_the_balance(): void
    {
        ...
        Deposit::create([
            'account_id' => $org->usage()->account()->getKey(),
            'credits' => 900,
            'reason' => 'gift',
        ]);

        $this->assertSame(950, $org->usage()->remaining());
    }
    ...
    public function test_an_adjustment_downwards_is_the_same_kind_of_row(): void
    {
        ...
        $org->usage()->deposit(-200, reason: 'adjustment', note: 'duplicate purchase');

        $this->assertSame(300, $org->usage()->account()->purchased_credits);
        $this->assertSame(2, $org->usage()->deposits()->count(), 'the history reads in one direction');

        // Clamped, not turned into a debt nobody can spend their way out of.
        $org->usage()->deposit(-9_999, reason: 'adjustment');
    ...
    public function test_a_deposit_records_where_it_came_from(): void
    {
        ...
        $deposit = $org->usage()->deposit(100, reason: 'purchase', source: $payment, note: 'inv 1234');
