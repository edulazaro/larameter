...
class WindowReadingTest extends TestCase
{
    ...
    public function test_one_window_answers_what_a_bar_needs(): void
    {
        ...
        $org->usage()->charge('thing', credits: 12_000);

        $weekly = $org->fresh()->usage()->in('weekly');
    ...
    {
        $org = $this->org();
        $org->usage()->charge('thing', credits: 1_000);

        $windows = $org->fresh()->usage()->windows();
    ...
    public function test_a_fixed_window_left_behind_reports_the_grid_it_is_on_now(): void
    {
        ...
        $org = $this->org();
        $org->usage()->charge('thing', credits: 5_000);
    ...
    public function test_an_expired_rolling_window_has_no_end_until_it_is_used_again(): void
    {
        ...
        $org = $this->org();
        $org->usage()->charge('thing', credits: 100);
        ...
        $this->assertNull($org->fresh()->usage()->in('session')->endsAt());
    }
